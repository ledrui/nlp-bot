...
class CosmosDbStorage {
    ...
    delete(keys) {
        ...
        return this.ensureCollectionExists().then(() => Promise.all(keys.map((k) => new Promise((resolve, reject) => this.client.deleteDocument(documentdb_1.UriFactory.createDocumentUri(this.settings.databaseId, this.settings.collectionId, cosmosDbKeyEscape_1.CosmosDbKeyEscape.escapeKey(k)), (err, data) => err && err.code !== 404 ? reject(err) : resolve()))))) // handle notfound as Ok
            .then(() => {
            return;
        }); // void
    }
...
function getOrCreateDatabase(client, databaseId, databaseCreationRequestOption) {
    ...
    return new Promise((resolve, reject) => {
        client.queryDatabases(querySpec).toArray((err, results) => {
            ...
            // create db
            client.createDatabase({ id: databaseId }, databaseCreationRequestOption, (db_create_err, databaseLink) => {
                if (db_create_err) {
                    return reject(db_create_err);
                }
